'use strict';
angular.module('jbehaveWebApp').controller('StoryCtrl', ['$scope','$http', 'filterFilter', function StoryCtrl($scope,$http,filterFilter) {
  $scope.stories = [];
  $scope.checkedStories = [];

  $scope.loadStories = function() {
   $http.defaults.useXDomain = true;
   $http({method: 'GET', url: 'http://localhost:8080/api/stories'}).
...
angular.module('jbehaveWebApp').directive('checkList', function() {
  return {
    scope: {
      list: '=checkList',
      value: '@'
